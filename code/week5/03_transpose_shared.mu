...
// Week5 版本强调 shared memory 的复用：global 连续读入 tile，再从 shared 转置写出。
// tile 第二维 +1 是经典 padding，避免转置读 shared 时多个线程打到同一个 bank。
...
int main() {
    ...
    std::printf("transpose shared padded %.3f ms\n", t.elapsed_ms() / 20.0f);
