...
每个示例文件都补充了统一的教学入口注释, 阅读时可以按以下顺序定位重点:

1. 先看文件顶部的“学习目标”和“阅读顺序”, 明确这个例子要解决的问题。
2. 再看“数据流”或 host/device、rank/device、stream/chunk 的关系。
3. 最后沿关键 API 调用阅读资源生命周期, 重点关注 bufferSize/workspace、同步边界和错误检查。

注释只解释会影响理解的设计选择和常见陷阱, 不逐行翻译代码; 文件中的片段或伪代码仍会明确标注不可直接编译。
