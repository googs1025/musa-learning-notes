...
照 `code/week1/Makefile` 抄,BINS 替换成 Week 2 的 8 个,MTCC / MUSA_PATH / LDFLAGS 全用变量,允许 `MTCC=... MUSA_PATH=... make` 覆盖。
...
| 风险 | 缓解 |
|---|---|
| API 字符串猜错(参数顺序、flag 名) | 已 SSH 上 AutoDL 列了头文件,核心 API 名全部存在;实际 signature 写完后会跑一次远程 `mtcc` 编译 sanity check |
