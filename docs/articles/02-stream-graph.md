...
> 这一周练习让 H2D / Kernel / D2H 重叠执行，提高程序吞吐量。
...
Week 2 把这些组合起来，目标是写出一个能让 H2D / Kernel / D2H 并行的 vectorAdd。过程中会处理三个问题：准确测量 kernel 时间，让 Async API 配合 pinned 内存工作，以及用 event 表达跨流依赖。最后再测量 MUSA Graph 能否减少 launch overhead。
...
运行环境和上周一样：AutoDL、MTT S4000、MUSA 3.1.0。下面记录几个实测后修正了原有认识的地方。
...
kernel 会异步入队，所以这段时间只包含 launch 开销。我写了 `03_vector_add_timer`，把三种计时方式并排跑了一遍：
...
- [B] 和 [C] 相差约 3 µs（2.4%），量级吻合，可以互相校验；
- [A] 比后两种结果小 30%。我原以为不 sync 会量到更多时间，实际上计时在 launch 完成后就停了，并没有等 GPU 算完；
- 这种"假数字"的可怕之处在于,它在 vectorAdd 这种几百 µs 的 kernel 上看着很合理,你不并排对比根本发现不了;
- kernel 越小,[B] 的 sync overhead 占比越大。真要测 µs 级 kernel,只能信 [C]。

从这一节开始，我把 PART III Q&A 中与毫秒数字相关的论断都标成 TODO，等实测后再回填。
...
pinned 内存的带宽接近这台机器 PCIe 的理论值（约 32 GB/s），pageable 内存只有它的四分之一。driver 需要先把 pageable 内存复制到内部的 pinned staging buffer，再用 DMA 传输，因此多了一次 host 内存搬运。

我在 NVIDIA 文档里见过 2 到 3 倍的类似结果，但亲自量到 3.81 倍还是有点意外。`musaMemcpyAsync` 遇到 pageable 内存时会退化成同步路径。多流流水线如果仍用 `malloc` 分配 host buffer，实际执行仍可能是串行的。
...
这台机器上的 MUSA 3.1.0 不支持 prefetch。wrapper 检测到 ENOSYS 后会自动跳过调用，因此“prefetch”版实际与“no prefetch”走同一条路。完整结果如下：
...
2. **生产环境使用统一内存要谨慎**。训练框架(torch_musa)默认使用 `musaMalloc` 和显式拷贝。UM 的 page fault 具有不确定性，会增大每个 step 的耗时方差，影响 batch / lr scheduler 的稳定节奏。
...
我把“加 event 拆 timeline”写成了习题 E2.9。在每个 stream 上记录 start / after-H2D / after-kernel / after-D2H 四个点，再算出每段的实际起止时间。画出甘特图后，才能判断瓶颈是 copy engine 数量，还是 kernel 调度被串行化。文章里先记录理论 3 倍、实测 1.14 倍的落差，具体原因留到习题中验证。

在拆开 timeline 之前，不能只根据理论值判断 stream 是否真正并行。
...
本次测试中，Graph 比 direct 慢 1.65 倍。我反复运行了几遍，verify 通过，数字也能稳定复现。这和我从 CUDA 形成的预期相反。
...
本次实测中，MUSA 3.1.0 的 `musaGraphLaunch` 每次约需 135 µs，比 5 次直接 launch 的总时间还长。我暂时不能确定原因，只能列出几个待验证的可能：
...
我的提交顺序是 0 → 1 → 2 → 3，完成顺序却是 0 → 3 → 2 → 1。第一眼看到时我以为是 bug，后来才意识到这正是跨 stream 无序执行的结果。哪个流上的 kernel 先跑完，对应的 callback 就先触发，与入队顺序无关。
...
文档还规定 callback 函数体里不能调用 MUSA API。driver 调用 callback 时仍持有 stream 调度锁，再次进入 driver 会死锁。这一点我还没亲自验证，习题 E2.8 用来观察具体报错。
...
- **用 musaEvent 测 kernel 时间**。CPU clock 在不 sync 时量到的是 launch 开销，sync 后的结果又包含 sync overhead，对 µs 级 kernel 不够准确。
